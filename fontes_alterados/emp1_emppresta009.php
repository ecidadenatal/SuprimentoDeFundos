  ...
  if (USE_PCASP) {
    ...
    try {
      ...
      if (isset($aprovacao)) {
        ...
        $oDaoEmpPrestaAprovacao = db_utils::getDao("empprestaaprovacao");
        $oDaoEmpPrestaAprovacao->sequencial = null;
        $oDaoEmpPrestaAprovacao->emppresta  = $e45_sequencial;
        $oDaoEmpPrestaAprovacao->observacao = $e45_obs;
        $oDaoEmpPrestaAprovacao->observacao_conferencia = $observacao_lancamento;
        $oDaoEmpPrestaAprovacao->aprovado   = $aprovacao == 1 ? 'true' : 'false';
        $oDaoEmpPrestaAprovacao->incluir(null);

        if ($oDaoEmpPrestaAprovacao->erro_status == 0 ) {
          $erro_msg = "Erro ao gravar a aprovação da prestação de contas.";
          $sqlerro  = true;
        }
