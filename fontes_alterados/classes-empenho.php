...
class empenho {
  ...
  function liquidar($numemp = "", $codele = "", $codnota = "", $valor = "", $historico = "", $sHistoricoOrdem='') {
    ...
    if ($e60_anousu < db_getsession("DB_anousu")){
      ...
      $codteste    = "33";
      $lRegistroEntradaRP = $oEmpenhoFinanceiro->empenhoRestosPagarPorDocumento(212, db_getsession('DB_anousu'));
      $lControleDespesaRP = $oEmpenhoFinanceiro->empenhoRestosPagarPorDocumento(214, db_getsession('DB_anousu'));
      if ( $lRegistroEntradaRP || $lControleDespesaRP) {
        $codteste = "39";
      }
  ...
  function empenho2Json($sWhere = '',$itens = 0, $aItensPendentesPatrimonio = array()){
    ...
    if ($this->getDados($this->numemp, $sWhere)){
      ...
      $strJson["iClassificacao"]  = $oEmpenhoFinanceiro->getClassificacaoCredor();
      $strJson["sClassificacao"]  = urlencode(ClassificacaoCredor::getDescricaoDaClassificacao($strJson["iClassificacao"]));

      $sDataVencimento = "";
      if (!empty($strJson["iClassificacao"]) && $strJson["iClassificacao"] != ClassificacaoCredor::DISPENSA) {

        $sDataVencimento = date("d/m/Y", db_getsession("DB_datausu"));
        $oDataVencimento = ClassificacaoCredor::getDataDeVencimentoPorData(new DBDate($sDataVencimento), $strJson["iClassificacao"]);
